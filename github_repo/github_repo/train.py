...
if optimizer_variant == 'adamw':
    ...
    regular_params = [p for p in model.parameters() if id(p) not in [id(p) for p in linear_modules]]
    param_groups = [
        {'params': regular_params, 'sn': False},  # use regular Adam update
        {'params': linear_modules, 'sn': True} # use Adam with subset norm step sizes
    ]
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        if losses['val'] < best_val_loss or always_save_checkpoint:
            ...
            if iter_num > 0:
                ...
                # Write final metrics for Part 2 report
                try:
                    import json
                    ...
                    metrics = {
                        ...
                        "max_iters": max_iters,
                        "train_loss": float(final_losses.get("train", float("nan"))),
                        "val_loss": float(final_losses.get("val", float("nan"))),
                    }
                    with open(os.path.join(out_dir, "final_metrics.json"), "w") as f:
                        json.dump(metrics, f, indent=2)
                    print(f"[metrics] wrote {os.path.join(out_dir, 'final_metrics.json')}")
                except Exception as e:
                    print(f"[metrics] failed to write final_metrics.json: {e}")
